...
- **Editor control from the AI collaborator:** Two Godot MCP servers are configured for this project, `godot-comprehensive` and `godot-coding-solo`, both pointed at the local Godot 4.7.1 executable. Together they can create projects, scenes, nodes, scripts, and resources, edit project settings, input maps, collision layer names, and autoloads, run the project, read debug output, and inspect and drive the running game's scene tree. Anything they create must still follow every rule in this document; document 28 defines when and how they are used, and destructive or outward-facing operations require the designer's confirmation in the main session. That confirmation is NOT enforced for delegated subagents: the Phase 00 connection matrix proved a subagent calls ask-gated tools with no prompt and no denial (Phase 00 finding F-06), so for delegated work the rule is enforced by instruction, not by the tool layer. Five tools are therefore denied outright rather than merely gated - Docker export, CI pipeline, HTTP request, WebSocket, and multiplayer server creation, the last because it binds all network interfaces rather than loopback - while file deletion and project export remain gated and so remain reachable by a subagent. Document 28 records the full limitation.
...
- **SimLoop order:** every physics tick runs, in order: 1 input · 2 player movement · 3 enemy AI and movement · 4 weapon targeting and firing (player, then Tower) · 5 projectile movement and sweep (enqueue hits) · 6 wind-up completion and contact ticks (enqueue hits) · 7 hit queue sorted by (target serial, attacker serial), player serial 0, Tower serial 1 · 8 death resolution in order Tower, bosses, player, other enemies · 9 drops · 10 pickup movement and collection · 11 XP and level-up requests · 12 Console channel completion (D115, 2026-09-23: the Tower Console is removed from runs; this step is kept as a named slot in `SimLoop.Step` for a possible future in-run interface, but nothing registers against it any more) · 13 Wave Director · 14 `PauseAuthority.flush()` · 15 UI state. Pause requests made during a tick apply at step 14; requests made between ticks (focus loss) apply immediately. A level-up requested at step 11 counts as an open Level-Up Draft for step 13 of the same tick, so the Wave Director defers any encounter due that tick.
- **Paused-menu timing (carve-out, Author decision D104):** gameplay timing runs on `SimClock` without exception. `SimClock` is `PROCESS_MODE_PAUSABLE`, so it stops for exactly as long as a pause lasts - which means a **paused menu cannot time itself on it**: the Level-Up Draft's 0.4 s input lockout, its 0.3 s cycle repeat and its 1.0 s hold-to-confirm all elapse while `SimClock.now` is frozen, and so do the pause menu's, the settings menu's and the run-end screens'. A paused-menu surface may therefore accumulate its own elapsed time in a `PROCESS_MODE_ALWAYS` node, and only for its own presentation timing. It may **never** use `get_tree().create_timer()` or `get_tree().create_tween()` (banned project-wide, and the banned-API check enforces it), never time gameplay state this way, and never let that accumulator outlive the menu. Everything the simulation can observe - cooldowns, wind-ups, lifetimes, spawn schedules - stays on `SimClock`. (The Tower Console's own dwell and purchase channel, cited here as the non-pausing counter-example, no longer exists in a run: D115, 2026-09-23, removed it.) Recorded because the rule as previously written was unsatisfiable rather than merely inconvenient: the Draft raised it as a contradiction (Phase 05 finding F05-11) instead of resolving it silently.
- **Physics & Collisions:**
...
| Console price per rank | integer Scrap | Retained as inert data (D115, 2026-09-23): priced the next rank at the Tower Console, which no runtime UI reads any more now that the Console is removed from runs; the field and its formula stay on the schema rather than being stripped out |
...
| Is unlock | bool | D118: true for a card gated behind a lifetime achievement, absent from every Draft pool query until `MetaProgress` reports it unlocked; false (the default) for a card offerable from a fresh profile's first run |
...
| Upgrade Console price formula | struct {Scrap per rank: integer, formula: price = Scrap-per-rank × rank being bought} | Retained as inert data (D115, 2026-09-23), same status as "Console price per rank" above |
