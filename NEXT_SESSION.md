...
5. **Push** main.
6. **GitHub release v0.1.0** (author asked: "the release ... exe in release section"). Match the author's own style (see their DiscRec repo release "v0.1.0 — Windows"): title `v0.1.0 — Windows`, attach `HordeControl.exe`, short body: what it is in one line, bullets (run loop, level-up drafts with rarity, skill tree + achievements, no installer, saves under %APPDATA%/Godot/app_userdata/Horde Control), requirements (Windows 10/11 64-bit, Vulkan GPU), credits line. `gh release create v0.1.0 builds/windows/HordeControl.exe -R Sayandeep1013/Horde-Control --title "v0.1.0 — Windows" --notes-file <file>`.
