...
- **The Tower Console:** The Console opens on the `console_open` action (keyboard E, gamepad Y / button index 3) while the player is inside the Interaction Radius **and** at least one entry is affordable, at any speed — Repair affordability follows C-REPAIR (docs/19 › Tower Console UI); a prompt shows while inside the radius, greyed when nothing is affordable (Author decision D107, 2026-09-23; see docs/19 › Tower Console UI for the full rule and docs/02 for the player-facing framing). With the Movement-only controls setting enabled, it instead keeps the ORIGINAL automatic open — stopped (speed under 10% of base) inside the Interaction Radius for 0.3 seconds with at least one entry affordable — since that setting's own no-button path cannot depend on a key or button (docs/19 › Input Map). It closes on leaving the radius, on Cancel (keys and buttons follow docs/19 › Input Map), on player death, or the instant a Level-Up Draft opens. Pressing `console_open` after a Cancel reopens the Console immediately, with no need to leave the radius and re-enter; under the Movement-only automatic open, a Cancel still locks that path until a genuine leave-and-reenter, unchanged. The Console is a **non-pausing, world-space interface**: the simulation keeps running, enemies keep attacking, and the Global Simulation Authority explicitly excludes it from the pause list. Because the left stick keeps moving the player, entries are cycled with the D-pad, the right stick, number keys, or the mouse, and each purchase is a 0.5-second channel with a fill ring — moving faster than 10% of base speed during the channel cancels it without spending anything; the movement-only alternative is sector selection, described in Platform Direction and the Tower Console UI.
- **The Vulnerability Window:** While the player's body overlaps the Tower Interaction Radius, the player's auto-fire is disabled at any speed (Provisional Default); the Tower keeps firing. The Tower Console itself opens on the `console_open` action at any speed (Author decision D107, 2026-09-23) — opening it and browsing its entries no longer requires stopping — but completing an actual purchase still requires the player to hold below 10% of base speed for the full 0.5-second channel; moving faster cancels it without spending anything. This keeps the core tension where it matters: entering the radius at all is what disarms auto-fire, and actually spending Scrap or repairing still costs standing still, exposed, for the channel's duration.
...
| Console rules test | P | The Console opens on the `console_open` action while inside the Interaction Radius with at least one affordable entry, at any speed (at 0 Scrap Repair is greyed and does not count toward opening the Console, C-REPAIR); with the Movement-only controls setting enabled, it instead opens automatically after 0.3 seconds inside the radius with player speed under 10% base and at least one affordable entry, unchanged from before D107. It closes on leaving the radius, Cancel, death, or a Level-Up Draft opening; pressing `console_open` after a Cancel reopens it immediately, while the Movement-only automatic open still stays closed after Cancel until the player leaves and re-enters. Every purchase channels for 0.5 seconds and cancels without charge above 10% base speed. | Scripted assertion | P2.13 |
...
| Tower Console dwell / auto-fire (Author decision D107, 2026-09-23) | Console opens on the `console_open` action (keyboard E, gamepad Y / button index 3) while inside the Interaction Radius with ≥ 1 affordable entry, at any speed; a prompt shows while inside the radius, greyed "(nothing affordable)" when nothing is affordable; with the Movement-only controls setting enabled, it instead keeps opening automatically after 0.3 s below 10% base speed inside the radius with ≥ 1 affordable entry, unchanged, since that setting's path needs no button; player auto-fire disabled at any speed while overlapping the radius, Tower keeps firing (C-AUTOFIRE; unaffected by D107 — see docs/19 Tower Console UI) | 19 |
...
| Meta: Run-End Settlement (prototype) | The prototype has one biome and no bosses, so its settlement pays: 1 Core per full minute of simulation time; 2 Cores per wave fully cleared; 1 Core per 25 enemies killed (floor); 5 Cores for clearing the final wave (the per-biome rate); then the Prospector node's bonus on the sum, rounded down. Settled once per run id, committed to disk before the run-end screen; abandon and window-close settle as failure (D110, D113) | 18, 24 |
...
| Value | Default | Owner document |
| --- | --- | --- |
| Tower Console rules (Author decision D107, 2026-09-23) | Opens on the `console_open` action (keyboard E, gamepad Y / button index 3) while inside the radius AND ≥ 1 entry affordable, at any speed (at 0 Scrap Repair is greyed and does not count toward opening the Console, C-REPAIR); a prompt shows while inside the radius, greyed "(nothing affordable)" when nothing is affordable; with the Movement-only controls setting enabled, opens automatically instead after 0.3 s inside the radius while player speed < 10% base AND ≥ 1 entry affordable — unchanged from the pre-D107 rule, since that setting's path needs no button; closes on leaving the radius, Cancel: Q / B/Circle (docs/19 › Input Map), death, or a Level-Up Draft opening; pressing `console_open` after a Cancel reopens the Console immediately with no need to leave and re-enter (the Movement-only automatic open still stays closed after Cancel until the player leaves and re-enters, unchanged); player auto-fire disabled at any speed while overlapping the Interaction Radius (C-AUTOFIRE), Tower keeps firing; every purchase is a 0.5 s channel with a fill ring, moving > 10% base speed cancels it without charge; input per the Input map, above; entries show price, "Rank n of 3", greyed if unaffordable, "MAX" with no price if maxed; the Console is a world-space Node2D under the gameplay root, z_index 38 (above enemies, below telegraphs), scale set each frame to the view scale so text stays ≥ 24 px tall on screen, nearest edge 200 px from the Tower's centre along the bearing opposite the player, flipping only after a 30° change, 85% opacity (C-CONSOLE-NODE); hidden and input-dead via PauseAuthority's `reasons_changed` signal whenever any pause reason is active, timers on SimClock | 19 |
...
| Input map (C-INPUT) | Full key/button mapping (Move, Draft cycle/select/confirm/reroll, Confirm, Console open/cycle/select/cancel, Pause) defined in docs/19 UI_UX › UI Layout & Dynamic Container Rules | 19 |
