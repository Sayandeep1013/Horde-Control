...
| Godot MCP servers drift from the pinned engine version | AI-driven scene, script, and settings edits behave differently than the pinned 4.7.1 engine expects, or silently stop working | Document 28 pins both Godot MCP servers' versions alongside the engine version (task P0.5); destructive or outward-facing MCP operations always require the designer's confirmation |
...
| Test | Tag | Pass condition | Instrument | First task |
| --- | --- | --- | --- | --- |
| Settings check | P | The exported Windows release build launches an empty scene with all 16 collision layer names, the full input map including number keys 1–7 and debug toggles, and pinned Godot 4.7.1 export templates present. | Scripted project audit | P0.2 |
