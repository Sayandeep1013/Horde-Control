...
| 0.8.7 | **Prototype-first redirection:** recorded four decisions taken at the author's direction to reach a visible, playable prototype with real assets before returning to accumulated irregularities. D97 defers the swarm stress test so it measures real entities rather than placeholder capsules. D98 narrows review to a spot-check during the push with a full gate on the finished prototype. D99 sets prototype art as AI-generated sprite sheets routed through the data contracts so the choice stays reversible. D100 fixes "prototype" as the Phase 03 slice, the smallest build that can answer the feel question. No gameplay rule, number, or gate changed; the Provisional Values Register is untouched. The engine-spine guarantees from Phase 02 - determinism, the pause authority, data-driven contracts, and the Register owning every number - are explicitly not relaxed by any of these. |
| 0.8.8 | **Remaining prototype functionality pass:** recorded three decisions taken as the prototype's remaining systems started. D101 records the CC0 asset route the author selected on 2026-09-19, which reverses D99's rejected alternative and was never written down at the time. D102 makes the art and asset pass its own session, after the remaining functionality and before the P2.16 external-tester gate, following the author's second play of the build. D103 repays the core simulation debt Phase 03 recorded (F03-06, F03-09, F03-17, F03-22, F03-26) now rather than at the Phase 06 gate, because six further systems occupy SimLoop steps 9 through 12 and would otherwise be built on top of it. No gameplay rule, number or gate changed; the Provisional Values Register is untouched. |
| 0.8.9 | **Paused-menu timing carve-out:** recorded decision D104 and wrote the carve-out into docs/20's Godot 4.x Implementation Standards, beside the SimLoop order bullet. The Level-Up Draft implementation surfaced that `SimClock`, being `PROCESS_MODE_PAUSABLE`, stops for exactly as long as a paused menu's own pause lasts, making "all timing on SimClock" unsatisfiable for that menu's input lockout, cycle repeat and hold-to-confirm. A paused menu may now accumulate its own presentation timing in a `PROCESS_MODE_ALWAYS` node; `get_tree()` timers and tweens stay banned, gameplay-observable timing stays on `SimClock`, and the Tower Console - which never pauses - is explicitly outside the carve-out. No gameplay rule or number changed; the Provisional Values Register is untouched. |
| 0.8.10 | **T4 teaching Siege tuned (P2.14).** The Register's T4 row carried a starting estimate it said would be tuned at P2.14; it now carries the measured result, Seekers 16 at 3 s with a 1.5 s interval, replacing 12 at 2.0 s. Measured over the Register's own tuning target, five seeds per half: with no player the Tower is destroyed in **5 of 5** seeds at 36-38 s of a 40 s wave (target: at least 3 of 5), and with a bot that reaches within 480 px of the Tower within 10 s and chases Seekers it ends at **100% health in 5 of 5** (target: above 50% in 5 of 5). The number was derived from the authored rates rather than found by trial: the Tower kills 25 DPS over 60 HP = 0.417 Seekers per second, arrival at 1/1.5 = 0.67 per second leaves a surplus that accumulates at the wall, and the handgun's 20 DPS lifts the combined kill rate above arrival so a responding player stops the accumulation. Two earlier candidates are recorded in the Phase 05 execution log: the authored 12 at 2.0 s destroys the Tower in 0 of 5, and 24 at 1.0 s destroys it in 5 of 5 but also kills it under a responding player, which teaches nothing. No intent changed - the tuning target itself is unaltered and was already the Register's own - so no Review Decision Log row is required. docs/11's restatement of the row is updated in the same commit. |
...
The Tower Console and its sector-selection paragraph once here are removed by D115 (no in-run shop, 2026-09-23); the **Movement-only controls** setting's remaining function is the Hub's Skill Tree screen (docs/18 › "Buying, and respec"), still offered as a hold-to-confirm choice on the run-end screens and in the pause and settings menus. Confirm, Dash, Reroll, and any other button are conveniences that make the same actions faster; none is required to complete a run.
...
Scrap is carried by the player and has no in-run sink (D115, 2026-09-23, removed the Tower Console and the in-run shop it offered); it converts to Cores at Run-End Settlement instead, and is lost if the player dies before then.
...
- **No in-run shop (Author decision D115, 2026-09-23):** the paragraphs this section once carried here — Processing & Upgrading, the Tower Console itself, the Vulnerability Window, No Safe Corners, and the "two channels" framing — described a `console_open`-triggered, world-space purchase/repair interface at the **Tower Interaction Radius** (a circle around the Tower base; Provisional Default 160 pixels in the prototype arena). That interface is removed from runs entirely: the only in-run power growth is the Level-Up Draft (Upgrade Channels, below), which is free, random, and pauses the simulation. Scrap has no in-run sink any more; it converts to Cores at Run-End Settlement instead (Provisional Values Register › "Meta: Run-End Settlement (prototype)"). The Tower Interaction Radius trigger area itself still exists in the scene (`src/tower/tower_interaction_radius.gd`) but has no consumer left; the removed paragraphs are preserved as history in the Review Decision Log's D115 row and are not restated here.
...
- **Tower health** does not regenerate passively. The Tower Console's Repair action this paragraph once described is removed (D115, 2026-09-23; no in-run shop); in-run Tower healing is now only the Repair Kit and Reinforced Plating Draft cards (Provisional Values Register › "Progression & Upgrades" › "Prototype upgrade pool").
...
Run-scoped power reaches the player through one channel. (Author decision D115, 2026-09-23, "no in-run shop": the Tower Console, the second channel this section used to compare the Draft against — priced, deterministic, a physical trip to the Tower — is removed from runs entirely; the table and rules that follow are retained below only where they still describe the Draft alone.)

| Level-Up Draft |
| --- |
| Trigger: XP level-up, opening immediately, mid-wave included, with no encounter deferral (Author decision D116, 2026-09-23) |
| Cost: Free |
| Selection: Three random cards from the upgrade pool, each rolling a rarity (Author decision D117; Provisional Values Register › "Draft rarity") |
| Simulation: Fully paused |
| Offers: Player upgrades and Tower upgrades; at least one of each in every draft |
| Extra actions: Reroll, 1 per run (prototype), 1 per draft (slice); Banish (slice only) |

Rules that follow from the table:

- Player upgrade cards and Tower upgrade cards must be distinguishable at a glance by frame shape, a fixed glyph, and a header word ("PLAYER" or "TOWER"), never by colour alone; rarity (Common/Rare/Epic) adds a border tint and a rarity label, itself never the only rarity signal (Provisional Values Register › "Draft card display").
- Tower upgrades taken in the Draft apply remotely — there is no "trip home" any more, since the Console that required one is removed. Every upgrade taken feeds the Tower's visual evolution count per its own `evolution_stage_contribution`.
- In the prototype the pool is eighteen upgrades (the original six plus the D115/D117 pool expansion), shared ranks, each with a stated maximum rank: Player — Rapid Fire (+10% fire rate/rank), Heavy Rounds (+10% damage/rank), Patch Kit (restores 30 health/rank taken), Swift Feet (+8% move speed/rank), Vitality (+15% max health/rank, healing the increase), Magnet (+25% pickup radius/rank), Regeneration (heal 1% max health/s per rank, stacks), Piercing Arrows (+1 pierced enemy/rank; UNLOCK, achievement-gated), Multishot (+1 extra arrow/rank at 70% damage, spread; UNLOCK); Tower — Caliber (+10% damage/rank), Optics (+15% range/rank), Shield Matrix (+15% max health as extra shield/rank), Reinforced Plating (+15% max health/rank, healing the increase), Watchtower (+10% range/rank), Tower Volley (+10% fire rate/rank; UNLOCK), Repair Kit (instantly heals 25% of current Tower max health, repeatable). A Draft slot guaranteed to an exhausted pool shows that pool's fallback card instead (Player "Overdrive", Tower "Reinforce"; see Default Unresolved-State Policies for the fallback rule). The three UNLOCK cards enter the pool only once their own lifetime achievement is unlocked (Provisional Values Register › "Meta: Achievements"); the full authored values live in Provisional Values Register › "Progression & Upgrades" › "Prototype upgrade pool", never restated a third time here.
...
| — | First reward | Level-Up Draft on the player's first level-up — organic, no longer fixed to land after the Siege (Author decision D108, 2026-09-23; see "Experience (XP)" and "A player never triggers a level-up during onboarding") | Investment is a choice |
...
Level-ups are no longer suppressed during T1–T4: a Level-Up Draft opens the instant accrued XP crosses the level curve's threshold, during a teaching wave exactly as during a combat wave, and shards are collected without an XP ceiling. This reverses D37's own reasoning (a level-up mid-teaching-wave would pause the simulation and interrupt the lesson) — see D108 for the alternatives weighed and why the faster level curve (Provisional Values Register › "XP & levels") makes the trade-off acceptable: the first Level-Up Draft now typically arrives naturally within the teaching waves, so the old forced-open-at-T4's-end mechanic is removed rather than kept alongside it.
...
**Experience (XP)** is a physical pickup called an XP shard, dropped by every standard enemy alongside Scrap (see Drop Table). XP shards obey the same magnet, cap, merge, and blocking rules as every other pickup. XP is not a currency: it has no sink other than the level bar. A run starts at level 0. Provisional Default level curve (Author decision D108, 2026-09-23, replacing the original 10 + 5(L+1)): advancing from level L to level L+1 costs 5 + 3(L+1) XP — 8, 11, 14 XP for the first three levels — and any XP earned beyond what a level-up consumes carries over as the remainder toward the next level. Each level-up opens one Level-Up Draft; simultaneous level-ups queue their drafts sequentially. Level-ups are no longer suppressed during teaching waves: a Level-Up Draft opens on the player's first level-up whenever it naturally occurs, including during T1–T4, exactly as it would during a combat wave (see "A level-up can now fire during onboarding," Onboarding Edge Cases; D108 replaces the earlier XP cap and forced first Draft entirely).
...
- **Sinks:** None *during the run* (D115, 2026-09-23, removed the Tower Console this line once named); converts to Cores at Run-End Settlement instead.
...
Kill → XP shard (level bar → Level-Up Draft) + Scrap (inventory → Run-End Settlement, D115: the Tower Console sink this line once named is removed) [+ Core if Elite or Boss (→ Meta Wallet)]. Scrap at cap → overflow hopper → Cores at a reduced rate on entering the Tower's Interaction Radius. Run end → Run Bonus in Cores. Player death → Scrap in hand and in the hopper lost, Cores already banked kept.
...
The player must always be able to spend. An economy state where the player has currency and nothing worth buying is a stall. In the prototype, where the pool is eighteen upgrades (D115/D117), the exhausted-pool fallback is the per-pool fallback card defined in Default Unresolved-State Policies. The in-run Tower Repair action this line once described (C-REPAIR, Tower Console UI) is removed by D115, 2026-09-23; Tower healing in-run is now the Repair Kit and Reinforced Plating Draft cards.
...
The interface specifications for the Level-Up Draft (the only upgrade channel remaining after D115, 2026-09-23), the HUD, the input map, and dynamic container rules live in `docs/19_UI_UX.md`, a working system document that is binding for the prototype under the Provisional Defaults Policy. The gameplay rules stay in Tower Overview and Player Overview.
...
The Tower Console (this paragraph's own former pause exception) is removed by D115, 2026-09-23; every remaining interface pauses fully except HUD tooltips, which do not pause.
...
The Tower Console this paragraph once named is removed (D115, 2026-09-23). HUD tooltips do not pause.
...
| Run ends while the Level-Up Draft is open | The interface closes without applying any pending choice; run failure resolves. (This row's own former Tower Console clause is removed, D115, 2026-09-23.) |
...
| Player is inside the Level-Up Draft when an encounter is scheduled | Encounter defers until the Draft closes plus the 1.5 second grace period. (This row's own former Tower Console clause is removed, D115, 2026-09-23.) |
...
- The Tower Console this bullet once described is removed (D115, 2026-09-23); ownership of the Level-Up Draft's own pause/encounter-deferral interaction stays with the Upgrade Draft UI & Navigation section alone.
...
| Does anything besides the Level-Up Draft pause the simulation in-run? | No. The Tower Console this row once asked about is removed (D115, 2026-09-23); only the Level-Up Draft and blocking system menus pause. |
...
| What does a movement-only player do at a menu? | In any paused menu, hold up toward the highlighted choice for 1.0 second to confirm it. (This row's own former Tower Console sector-selection clause is removed, D115, 2026-09-23; the Movement-only controls setting's remaining function is the Hub's Skill Tree screen's stand-still-buy path, docs/18.) |
...
| What happens if a Level-Up Draft opens while the Tower Console is open? — removed (D115, 2026-09-23) | The Tower Console no longer exists in a run; the Level-Up Draft is the only upgrade interface. |
...
Owns: all Tower-specific edge cases, the Tower Targeting Rule, shield regeneration timing, drone persistence across biomes, Tower Interaction Radius rules, and the evolution stage thresholds. The Tower Console mechanics and Tower Repair's in-run price this row once owned are removed by D115 (no in-run shop, 2026-09-23); Tower healing in-run is now the Repair Kit and Reinforced Plating Draft cards only (Register > "Progression & Upgrades").
...
Owns: the XP level curve, Reroll, Banish, simultaneous level-up queueing, exhausted pool fallbacks, and level-up during boss transitions.
...
- It behaves correctly under pause, the Level-Up Draft (the only upgrade interface after D115, 2026-09-23), and, where the milestone includes them, biome transitions.
...
| Interaction window test — removed (D115, 2026-09-23) | P | The Tower Console is removed from runs; there is no interaction-radius purchase window left to test. | — | — |
...
| Channel understanding test — removed (D115, 2026-09-23) | VS | The Tower Console is removed; the Level-Up Draft is the only upgrade channel, so there is no second channel left to compare it against. | — | — |
...
| Guaranteed first draft test | P | The first Level-Up Draft opens the instant the player's XP first reaches the level-1 cost, even during a teaching wave (T1–T4), in 5 of 5 runs — level-ups are no longer suppressed or capped during onboarding (Author decision D108, 2026-09-23). | Scripted test | P2.12 |
...
| Console non-pause test — removed (D115, 2026-09-23) | P | The Tower Console is removed from runs; there is no non-pausing interface left to test. | — | — |
| Console rules test — removed (D115, 2026-09-23) | P | The Tower Console is removed from runs; its lifecycle/purchase-channel rules no longer apply. | — | — |
...
| XP cap check | P | During T1–T4, XP is never artificially capped and level-ups fire normally, exactly as during a combat wave; a shard burst that crosses a level threshold mid-teaching-wave applies the level-up immediately, with no suppression and no revert (Author decision D108, 2026-09-23 — the earlier 14-XP cap and forced T4-end Draft are both removed). | Scripted assertion | P2.12 |
...
Teaching wave — one of the four onboarding waves (T1 through T4) that open a run's first biome before its combat wave 1; not counted among the biome's eight combat waves. Runs its own C-TEACH timing rules (never the stall rule, Overtime, or escalation); no longer suppresses level-ups or caps XP (Author decision D108, 2026-09-23 — see "XP & levels").
...
Draft — short for Level-Up Draft, defined below (the only upgrade interface after D115, 2026-09-23 removed the Tower Console).
...
Scrap — the run currency: dropped by standard enemies, elites, and bosses, capped at 200, with no in-run sink (D115, 2026-09-23, removed the Tower Console this entry once named as the spend point) — it converts to Cores at Run-End Settlement instead; Scrap collected beyond the cap enters the overflow hopper in the vertical slice (discarded, with a FULL indicator, in the prototype); any Scrap held is lost on death.
...
Tower Console — removed (D115, 2026-09-23). Formerly the priced, deterministic, non-pausing upgrade and repair interface inside the Tower Interaction Radius; the Level-Up Draft is now the only upgrade interface.
...
Vulnerability window — removed with the Tower Console (D115, 2026-09-23). Formerly the state of having the Console open: auto-fire disabled, still damageable.
...
Sector selection — removed with the Tower Console (D115, 2026-09-23). Formerly the movement-only purchase method in the Console when the Movement-only controls setting is on.
...
- The Tower Interaction Radius, which disables the player's auto-fire at any speed while overlapped (see Tower Interaction Mechanics) -- the Tower Console and its vulnerability window this bullet once also named are removed (D115, 2026-09-23).
...
8. The Level-Up Draft pauses the simulation completely (Pause authority test). The Tower Console is removed (D115); this criterion's own former second clause (Console non-pause test) no longer applies.
...
| Tower Console dwell / auto-fire — removed (D115, 2026-09-23) | The Tower Console is removed from runs (no in-run shop; Author decision D115). `console_open`, its dwell/auto-open rule, and its Interaction-Radius prompt no longer exist. C-AUTOFIRE (player auto-fire disabled while overlapping the Interaction Radius) was never wired to any live signal even before D115 (`src/combat/auto_weapon.gd`'s own header named the gap) and remains unwired; the Interaction Radius trigger itself (`src/tower/tower_interaction_radius.gd`) is untouched but has no consumer left | 19 |
| Tower evolution thresholds | 0 / 1 / 3 / 6 ranks (Base / Reinforced / Armed / Fortress) | 07 |
| Tower Repair price — removed (D115, 2026-09-23) | The in-run Repair action was the Console's own action and is removed with it. Tower healing in-run is now only the Repair Kit Draft card (Register > "Progression & Upgrades" > "Prototype upgrade pool") and Reinforced Plating's heal-the-increase; the Tower still has no passive regen | 07 |
...
| XP & levels (Author decision D108, 2026-09-23; was 10 + 5(L+1)) | 1 XP shard per standard enemy; start level 0; level L→L+1 costs 5 + 3(L+1) XP (8, 11, 14, 17…), remainder carries | 13 |
| Teaching wave XP (C-XPCAP) — removed (Author decision D108, 2026-09-23) | No longer suppresses level-ups or caps XP during T1–T4; a level-up fires normally in a teaching wave exactly as in a combat wave, with no XP ceiling; the forced first Draft at T4's end is removed too, since the faster level curve above lets the first Draft arrive naturally within the teaching waves. This reverses D37's own rationale; see the Review Decision Log's D108 row for the alternatives weighed | 13, 02 |
...
| Value | Default | Owner document |
| --- | --- | --- |
| Prototype upgrade pool (D115/D117, 2026-09-23: expanded, and the three former +20%/rank cards move to +10%, D117's Common base value) | Player — Rapid Fire +10% fire rate/rank; Heavy Rounds +10% damage/rank; Patch Kit restores 30 health/rank taken; Swift Feet +8% move speed/rank (3 ranks); Vitality +15% max health/rank, healing the increase (3 ranks); Magnet +25% pickup radius/rank (2 ranks); Regeneration heal 1% max health/s per rank, stacks (5 ranks); Piercing Arrows +1 pierced enemy/rank (3 ranks; UNLOCK, D118 achievement-gated); Multishot +1 extra arrow/rank at 70% damage, spread (3 ranks; UNLOCK). Tower — Caliber +10% damage/rank; Optics +15% range/rank; Shield Matrix +15% max health as extra shield/rank; Reinforced Plating +15% max health/rank, healing the increase (3 ranks) — the author's own "Reinforce" renamed to avoid colliding with the existing Tower damage fallback card of that name; Watchtower (draft card) +10% range/rank (3 ranks; shares its name with the permanent Skill Tree node of the same effect, a deliberate flavour reuse per the author's own naming, not a collision — the two are never shown on the same screen); Tower Volley +10% fire rate/rank (3 ranks; UNLOCK); Repair Kit instantly heals 25% of current Tower max health, repeatable (max rank 10, a practical cap the pool-membership query requires rather than true infinity). Max rank 3 each unless stated otherwise above, shared ranks | 17 |
| Console price — retained as inert data only (D115, 2026-09-23) | `UpgradeDefinition.console_price_per_rank` (30 Scrap × rank being bought) stays as an authored Content Data Contract field (no Register-visible UI reads it any more, since the Console is removed); `UpgradeSystem.get_console_cost()` is unused by any production caller | 14, 17 |
| Fallback cards (C-FALLBACK-CONSOLE) | No max rank, add 0 evolution ranks; Player "Overdrive +10% weapon damage", Tower "Reinforce +10% Tower damage"; appear in the Draft for an exhausted pool, so Scrap-priced content still has a sink even though the Console itself is gone (D115) | 17 |
| Level-Up Draft (D116, 2026-09-23) | Pauses fully; 3 cards in a row, background dimmed 60%; ≥ 1 player + ≥ 1 Tower card; reroll replaces all 3, keeps the guarantee, avoids the 3 just shown when the pool allows; Reroll 1 per run (prototype) / 1 per draft (slice); Banish slice only; no Cancel; opens the instant XP crosses the level-cost threshold, mid-wave included, with no encounter deferral; simultaneous level-ups queue their drafts back to back | 13 |
| Draft card display | Icon, name, one sentence, rank change "Rank 1 → 2 of 3"; differentiation by frame shape, fixed glyph, header word, never colour alone; a rarity badge and border tint (D117, see "Draft rarity" below) | 13, 19 |
...
| Draft rarity (D117, 2026-09-23) | Every rolled Draft card gets a rarity — Common (value ×1.0), Rare (×1.5), Epic (×2.2) — multiplying the card's own `effect_per_rank` (Common base values: percentage upgrades +10%, per the values in "Prototype upgrade pool" above). Starting odds at player level 0: Common 75% / Rare 21% / Epic 4%; each player level shifts 1.5 points from Common to Rare and 0.5 from Common to Epic, capped at Common 45% / Rare 40% / Epic 15%. The Fortune "Lucky Charm" skill node (Register > "Meta: Skill Tree effects") adds flat luck points that shift the odds identically to a level. Rolled with the project's own Keyed RNG (`[run_seed, "draft_rarity", k]`, a serial separate from the card-pick roll's own `k`), so the same run seed reproduces the same rarities | 13, 17 |
...
| Meta: Run-End Settlement (prototype) | The prototype has one biome and no bosses, so its settlement pays: 1 Core per full minute of simulation time; 2 Cores per wave fully cleared; 1 Core per 25 enemies killed (floor); 5 Cores for clearing the final wave (the per-biome rate); floor(Scrap carried / 10) Cores (D115: the Console's removal leaves Scrap with no other sink, so whatever reaches settlement — the Scrap cap and loss-on-death rules still apply — converts here instead of being discarded); then the Prospector node's bonus on the sum (including the Scrap line), rounded down. Settled once per run id, committed to disk before the run-end screen; abandon and window-close settle as failure (D110, D113) | 18, 24 |
| Meta: Skill Tree costs | Rank r of a tier-t node costs base(t) x (1 + 0.5 x (r - 1)), rounded; base 5 / 10 / 18 / 30 Cores for tiers 1-4; the root (Command Tent) is free and always owned; respec is free and full, Hub only (D111); Core wallet clamped at 999,999 | 18 |
| Meta: Skill Tree effects | Vitality +10% player max health (3 ranks); Swift Boots +4% move speed (3); Sharpened Arrows +8% player weapon damage (3); Quick Draw +5% fire rate (3); Long Reach +20% magnet radius (2); Second Wind: once per run a lethal hit leaves the player at 30% health (1); Stone Walls +10% Tower max health (3); Arrow Slits +10% Tower weapon damage (3); Shield Runes +15% Tower max shield (2); Mason's Kit +10% Tower shield regen rate per rank (2) — repurposed by D115 (2026-09-23) from its former -15% Tower Console repair price, since the Console it discounted no longer exists; Watchtower +10% Tower weapon range (2); Fortress: Tower starts one evolution stage up and gains +20% max health and +20% weapon damage (1); Scavenger: start with 15 Scrap per rank (3); Scholar +10% XP from shards (3); Lucky Draw +1 Draft reroll per run (2); Prospector +15% settlement Cores (2); Lucky Charm +3 rarity-luck points per rank (2), shifting Draft card rarity odds identically to a player level (D117, 2026-09-23; see "Draft rarity" above) — a side node off Scavenger, not part of the Lucky Draw/Prospector pair Deep Pockets requires; Deep Pockets +50 Scrap carry cap (1); War Chest: start at level 1 with one free Draft when the first wave begins (1). Percentages add within a node and multiply the base once; no effect exceeds a cap stated elsewhere in this Register | 18 |
| Meta: Save profile | `user://profile.json`, schema_version 2 (D118, 2026-09-23: adds `lifetime_kills`, `lifetime_scrap_collected`, `unlocked_achievement_ids`; a schema_version 1 or absent-key profile migrates to 2 with all three at zero/empty, per the `migrate_N_to_N_plus_1` chain in document 24); atomic write via .tmp then rename with a .bak of the previous file; corrupt file falls back to .bak, then a fresh profile with profile.corrupt.json kept; saves at settlement, each purchase, each respec, and on close outside a run; settled run ids kept: last 16 | 24 |
| Meta: Achievements (D118, 2026-09-23) | Six lifetime/per-run goals, evaluated once per run at Run-End Settlement: Goblin Slayer (300 lifetime kills) unlocks the Piercing Arrows Draft card; Veteran (reach wave 5 in one run) unlocks Multishot; Keeper of the Keep (end a run with the Tower above 50% health) unlocks Tower Volley — these three are exactly the pool's three UNLOCK-gated cards (Register > "Progression & Upgrades" > "Prototype upgrade pool"), so every card an achievement can unlock is gated by exactly one achievement. Hoarder (500 lifetime Scrap collected) grants a permanent +1 Level-Up Draft reroll per run (composes with Lucky Draw); Founder (win a run) grants +5% Run-End Settlement Cores (composes with Prospector); Marksman (1000 lifetime kills) grants +5% player weapon damage (composes with Sharpened Arrows) — none of the three unlocks a card, since every pool card an achievement could plausibly unlock is already claimed by the first three, and an achievement "unlocking" a card the pool already offers from a fresh profile would be a lie. A newly unlocked achievement is listed on the run-end results screen and on the Hub's Achievements panel; already-unlocked achievements are never re-reported | 18 |
...
| Value | Default | Owner document |
| --- | --- | --- |
| Tower Console rules — removed (D115, 2026-09-23) | The Tower Console (Author decision D107) is removed from runs entirely: no `console_open` action, no world-space panel, no priced purchase channel, no Repair action. The only in-run power growth is the Level-Up Draft. See docs/19 › "Tower Console UI" (marked removed) | 19 |
| Movement-only controls setting (C-SECTORS) | Default off; offered as a hold-to-confirm choice on run-end screens and pause/settings menus. D115 (2026-09-23) removed the Console, so this setting's former seven-sector Console purchase path (Repair, Rapid Fire, Heavy Rounds, Patch Kit, Caliber, Optics, Shield Matrix) no longer applies; the setting's remaining function is the Hub's Skill Tree screen's own movement-only stand-still-buy path (docs/18 › "Buying, and respec") | 19 |
...
| Input map (C-INPUT) | Full key/button mapping (Move, Draft cycle/select/confirm/reroll, Confirm, Pause) defined in docs/19 UI_UX › UI Layout & Dynamic Container Rules. The eleven `console_*` actions are removed with the Tower Console (D115, 2026-09-23) | 19 |
...
| Readability | Degradation from the bottom of the hierarchy upward when > 24 high-intensity effects are visible or 1st-percentile FPS < 45; draw order z_index: environment 0, pickups 10, the play layer 20 (player, Tower, and enemies share one Y-sorted band — an entity or building whose feet/footprint sit further north (smaller y) draws behind one further south, regardless of kind — Author decision D119, 2026-09-23, replacing the earlier fixed Tower 25 / player 50 bands), player projectiles 30 (≤ 70% opacity), effects 35, telegraphs 40, overhead player/Tower health, shield, and XP bars 50 (always drawn above the play layer regardless of Y-sort — Author decision D119), damage numbers 60 | 27 |
...
| Value | Default | Owner document |
| --- | --- | --- |
| Teaching wave budgets | T1 20 + gap 5 + T2 25 + gap 5 + T3 30 = 85 s, within the restored ninety-second onboarding limit for the five lessons; + gap 8 (Split recovery gap) + T4 40 → T4 ends ≈ 133 s after launch; the 10 s Siege recovery gap follows T4's end; the first Level-Up Draft is no longer forced open at T4's end — it opens organically on the player's first level-up, which the faster level curve typically brings within the teaching waves themselves (Author decision D108, 2026-09-23; "XP & levels"); Tower understood by end of T2 ≈ 50 s | 02, 11 |
...
| D37 | No level-up fires while T1–T4 are open, and XP is capped at 14 during them | Let level-ups fire normally during onboarding | A level-up mid-teaching-wave would pause the simulation and interrupt the lesson before it lands; capping XP and forcing the first Draft only after T4 lets onboarding finish before the player's first real choice. **Superseded by D108 (Author decision, 2026-09-23):** level-ups now fire normally from wave 1, including during T1–T4; the faster level curve D108 also introduces makes the trade-off this row accepted no longer necessary |
...
| D97 | Author decision (2026-09-18): the project pushes to a visible, playable prototype with real assets and animation before returning to fix accumulated irregularities. P1.7, the 300-enemy swarm stress test, is **deferred** rather than dropped, and runs against the Phase 03 entities instead of against placeholder capsules | Run P1.7 now as the plan sequences it, holding the prototype until the engine budget is validated; or drop the swarm test and rely on the Performance Fallback Ladder reactively | Measuring 300 placeholder `CharacterBody2D` capsules answers a question about capsules. The same test against real enemies - with sprites, animation, hitboxes, a weapon firing at them and pickups dropping - measures what the budget actually has to hold, and by then it is cheaper to stage because the entities exist. The accepted risk is that the 300-enemy budget stays unvalidated longer; that risk is bounded because the Fallback Ladder is a pre-agreed response rather than a redesign, and because the prototype will surface a gross performance problem long before a formal test would |
| D98 | Author decision (2026-09-18): during the prototype push, implementation tasks take a **spot-check** rather than a per-task critical agent, and the full review gate runs once against the finished prototype. This narrows D83 and D94 for this stretch of work only | Keep D94's four-critical-agents-plus-phase-reviewer pace through the prototype push; or suspend review entirely until the prototype is complete | D83 makes review rigour a function of risk and requires any narrowing to be decided in advance and written down, which is what this row does. The prototype push is exploratory work whose purpose is a feel verdict, and a feel verdict is not something a blind reviewer can supply - the author plays it. Suspending review entirely was rejected because the engine spine underneath is load-bearing and its guarantees still need defending; the spot-check keeps that floor |
| D99 | Author decision (2026-09-18), **amended the same day because the chosen route proved unavailable**: the author selected AI-generated sprite sheets; the configured image service requires a paid plan this account does not hold and rejected all eight submissions, so nothing was generated and nothing was charged. Prototype art is instead produced by a committed deterministic generator, `tools/art/generate_sprites.py`, directed to satisfy the readability hierarchy - silhouette class, reserved colour per entity type, and minimum on-screen size. Every asset path is a field on the relevant `.tres` contract rather than a hardcoded reference | Use a CC0 asset pack such as Kenney's, which is licence-clean and instantly consistent; or use styled programmatic primitives, which satisfy the readability spec by construction and carry no licence surface at all | The substituted route serves the original intent better than the one it replaced, which is why it was not escalated as a scope change: the art is reproducible from source and reviewable as a diff rather than an opaque binary drop, it carries no third-party licence, and it satisfies the readability rules by construction rather than by steering a model toward them. A `--silhouette` mode renders every entity as flat black so the distinguishability claim can be checked rather than asserted. **Stated plainly: the result is stylized geometric art, not finished character art.** It is sufficient to answer the feel question Phase 03 exists to ask, and no further. Generated art gives direct control over the one thing the master actually constrains - that a Tower Seeker, a Player Hunter and an Opportunist stay distinguishable at a glance under load - where a stock pack would have had to be recoloured toward the same rules anyway. Routing every path through the data contracts means the choice is reversible as a data edit: commissioned or purchased art later replaces a field value, not a code path. The licence terms of the generator are the author's to accept |
| D100 | Author decision (2026-09-18): "a working prototype" means the Phase 03 slice - player controller, arena and camera, the handgun with auto-targeting, the Tower, one enemy per target intent, and the HUD - played end to end, rather than the full Minimum Playable Prototype of Phase 06 | Push straight to the Phase 06 prototype gate with wave direction, upgrades and run flow included; or stop at a narrower movement-and-arena demo | Phase 03 is the first point at which the Central Tension is actually playable: two health pools, enemies that want different things, and a weapon that fires itself. That is the smallest build that can answer the feel question, and the feel question is what the author needs answered. Phase 06's scope adds the Wave Director, both upgrade channels and run flow - real work that does not change the answer to "does this feel right" |
| D101 | Author decision (2026-09-19, recorded here 2026-09-20): prototype art comes from free, CC0-licensed online asset packs, committed with the licence file each pack ships, rather than from the generated geometric sprites D99 settled on; the generator and its sprites stay in the tree as the fallback that proves entity distinguishability with colour removed | Keep the generated geometric sprites (D99's outcome), or pay for the image service D99 selected and could not use | D99's own row names "use a CC0 asset pack such as Kenney's" as the alternative it rejected, so selecting it is a reversal and needs its own row rather than an edit to D99. Recorded late: the work ran in the 2026-09-19 session and the row was proposed there but never written |
| D102 | Author decision (2026-09-20): the art and asset pass is its own dedicated session, run after the prototype's remaining functionality lands and before the external-tester gate at P2.16; until then the stand-in art is expected to read as unfinished and is not re-opened task by task | Fix art continuously inside every functional task, or bring Phase 11's production-art task forward now | The author played the wave build and recorded "the movement is good and its shooting ... the assets are severely lacking ... we need to do a proper session on just that ... mark that for later". Art spread across functional tasks is what produced three separate asset findings in Phase 03 (F03-21, F03-31, F03-32), and the P2.16 external testers are the first audience for whom the art has to carry itself |
| D103 | Orchestrator decision (2026-09-20), open to the author's review: the core simulation debt Phase 03 recorded - F03-09 (entities step themselves rather than being driven by SimLoop), F03-22 (melee and contact damage resolves inside the overlap callback instead of through the hit queue), F03-17 (the Tower's projectile has no sweep), F03-06 (the player's death is broadcast as an enemy kill) and F03-26 (the Tower is invisible to the EntityRegistry) - is repaid now, alongside the first wave of remaining prototype systems, rather than at the Phase 06 gate | Carry the debt to the gate and build the six remaining prototype systems on top of it | Six further systems (pickups, XP, upgrades, Draft, Console, run flow) occupy SimLoop steps 9 through 12. Each one built before the repair is another system that steps itself and resolves damage outside the queue, and F03-22 is already a Major that three separate tasks have repeated |
| D104 | Author decision (2026-09-20): the rule that all timing runs on `SimClock` gains one explicit carve-out, written into docs/20. A **paused menu** - the Level-Up Draft, the pause menu, the settings menu, the run-end screens - may accumulate its own presentation timing in a `PROCESS_MODE_ALWAYS` node, because `SimClock` is `PROCESS_MODE_PAUSABLE` and stops for exactly as long as the menu's own pause lasts. `get_tree().create_timer()` and `create_tween()` stay banned; everything the simulation can observe stays on `SimClock`; the Tower Console is not a paused menu and is not covered | Add a second `UiClock` autoload at `PROCESS_MODE_ALWAYS` for every paused menu (rejected: docs/20 defines exactly four autoloads and that list has been treated as binding), or make `SimClock` itself `PROCESS_MODE_ALWAYS` and have every gameplay system ignore its advance while paused (rejected: it moves the pause guarantee out of the engine and into each system's discipline, which is the determinism spine Phase 02 was built around) | The rule as written was **unsatisfiable**, not merely awkward: a menu whose own pause stops the only clock it is allowed to read cannot time its own input lockout. The carve-out is the smallest change that makes the rule true, and writing it down is what separates it from the local workaround the Draft would otherwise have carried undocumented |
| D105 | Author decision (2026-09-23), executing D101 and D102: the art session adopts Pixel Frog's **Tiny Swords**, the edition its store page labels CC0 (`assets/third_party/tiny_swords/PROVENANCE.md`), as the single visual language - goblin troops for the three enemy intents in their reserved colours (Torch Red for the Tower Seeker, TNT Yellow for the Player Hunter, Barrel Purple for the Opportunist), the blue Archer for the player, Knights buildings for the Tower's evolution stages, and a grassland-island biome - with frame-animated run, strike, hit and death states; music is cynicmusic's CC0 "Battle Theme A" | Keep the Kenney CC0 set with procedural animation (no frame animation exists in it); use Ninja Adventure (CC0, but 16 px and monsters carry walk cycles only); or the newer Tiny Swords Free Pack, whose licence forbids redistribution | Tiny Swords is the only candidate found that is redistributable and has run, attack and death frames for a whole enemy roster, a building set for the Tower, and matching terrain |
| D106 | Author decision (2026-09-23): the UI pass branch (`ui-pass`, restyle and runtime string registration) is merged into `main`, and the debug overlay starts hidden in the launched build, F1 still toggling it | Leave the branch for a later merge and keep raw `tr()` keys on screen; keep the overlay visible by default | The build is meant to be played by people who are not developers |
| D107 | Author decision (2026-09-23): the Tower Console opens on a new `console_open` action (keyboard E, gamepad Y / button index 3), while the player is inside the Interaction Radius with at least one entry affordable, at any speed; a prompt shows while inside the radius, greyed to "(nothing affordable)" when nothing is. With the Movement-only controls setting enabled, the Console keeps its ORIGINAL automatic 0.3 s dwell-open (speed-gated, unchanged) instead, since that accessibility path cannot depend on a button (docs/19's own Input Map: "every menu has a movement-only path ... require none of the keys or buttons"). After a Cancel, `console_open` reopens the Console immediately with no need to leave the radius and re-enter; the Movement-only automatic open keeps its own leave-and-reenter lock after a Cancel, unchanged | Keep the automatic dwell-open for every control scheme with a clearer/more visible prompt, rather than moving to a button for the default scheme | Players experienced the automatic dwell-open as a random pop-up (it fires the instant the player happens to stand still near the Tower, regardless of intent). A deliberate keypress removes that surprise while the prompt keeps discoverability; keeping the automatic open exclusively for Movement-only mode preserves that setting's own no-button guarantee without inventing a second undocumented mechanism |
| D108 | Author decision (2026-09-23): the level-cost curve changes from 10 + 5(L+1) XP to 5 + 3(L+1) XP (8, 11, 14, 17… for the first four levels); the teaching-wave XP suppression and 14-XP cap (C-XPCAP) are removed, so a level-up fires normally during T1–T4 exactly as during a combat wave; the forced first Level-Up Draft at T4's end is removed as well, since the faster curve typically brings the player's first level-up naturally within the teaching waves themselves | Keep the teaching-wave suppression block alongside the faster curve (a level-up would still wait until T4 ends, just with more XP banked by then); or only halve the level curve (10+5(L+1) → 5+2.5(L+1)) without touching the teaching-wave suppression or the forced draft at all | Faster levelling on its own, with the old suppression left standing, would mean MORE XP sits capped and discarded during onboarding, not less — the two changes have to move together. Removing the forced draft rather than keeping it alongside an organic one avoids TWO first-Draft mechanisms fighting over "which one is really first"; this reverses D37's own rationale (a level-up mid-teaching-wave interrupting the lesson), a trade-off the author accepted in favour of the earlier, more natural choice point this task brief itself recommended |
| D109 | Orchestrator decision (2026-09-23) under the author's standing instruction to take the recommended option while away: the prototype gains the meta layer now - a Hub (War Camp) between runs, a fog-revealed grid Skill Tree of 18 ranked nodes in three branches bought with Cores, a results screen fed by a committed Run-End Settlement, records, and an atomic, versioned save profile (docs/18, docs/24). Title Play now opens the Hub | Keep the prototype meta-free as the master's Phase Map intended (Hub and tree at the vertical slice) | The author asked for the skill tree and roguelite elements explicitly, and a run that leaves nothing behind is the biggest reason a short-run game is not replayed |
| D110 | Orchestrator decision (2026-09-23): the prototype's Run-End Settlement adds 2 Cores per wave cleared and 1 per 25 kills to the master's 1 per minute, and pays the per-biome 5 for clearing the final wave, because the prototype has one biome and no bosses | Use the master's rates unchanged (about one Core per minute of a failed run) | Unchanged rates make a failed first run buy nothing; the target is two or three root-adjacent nodes after the first run |
| D111 | Orchestrator decision (2026-09-23): Skill Tree respec is free and full, in the Hub only | Charge a Core fee per respec, as Rogue Legacy does | A free refund makes experimenting safe and is the dominant pattern in the genre (Vampire Survivors, Brotato) |
| D112 | Orchestrator decision (2026-09-23): a fifth Autoload, `MetaProgress`, owns the profile, the wallet, tree ranks, settlement and saving, alongside SimClock, PauseAuthority, EventBus and EntityRegistry | A static class with cached state, or passing a profile node through every scene change | The profile must survive scene changes between the title, the Hub and runs, and one owner with one save function is what makes the atomic-write and idempotence rules enforceable |
| D113 | Orchestrator decision (2026-09-23): closing the window during a run is treated as an abandon - settle as a failure, save, then quit | Treat a window close like a crash, paying nothing | A deliberate close is not a crash; losing a run's earnings to the window's close button feels like a bug to the player |
| D114 | Orchestrator decision (2026-09-23), from the blind review of the meta layer: the Fortress capstone keeps its one-stage evolution head start and adds +20% Tower max health and +20% Tower weapon damage, because the evolution stage alone changes only the Tower's art in the prototype | Keep Fortress cosmetic and lower its price; or disclose it as a placeholder | A 30-Core capstone that changes nothing a player can feel reads as a bug; the added numbers sit alongside Stone Walls and Arrow Slits rather than exceeding them |
| D115 | Author decision (2026-09-23): no in-run shop. The Tower Console is removed from runs; the only in-run power growth is the Level-Up Draft. Scrap stays as run loot and converts to Cores at Run-End Settlement at 10 Scrap per Core (floor); the Scrap cap and loss-on-death rules still apply to what reaches settlement | Keep the Console for repair only; or keep it for both upgrades and repair | The author: "player shouldn't be able to upgrade any specific things like those while in a run other than the random 3 generated upgrades"; converting Scrap keeps its pickups meaningful |
| D116 | Author decision (2026-09-23): a Level-Up Draft opens the moment the XP bar fills, mid-wave included, pausing the game; the choice applies immediately and XP beyond the level cost carries into the next level. The encounter deferral of Drafts is removed | Keep deferring Drafts to recovery gaps between encounters | The author asked for the fill-pause-choose loop to happen during a wave; carrying the remainder is the genre norm and avoids wasting XP |
| D117 | Orchestrator decision (2026-09-23) under the author's direction that drafts 'won't be truly random': Draft cards roll a rarity - Common (base value), Rare (x1.5), Epic (x2.2). Starting odds 75/21/4; each player level in the run moves 1.5 points from Common to Rare and 0.5 to Epic, capped at 45/40/15; a Fortune skill node adds luck. Common base values are +10% for percentage upgrades (the author's example) | Flat uniform cards; or rarity by wave instead of level | Rarity that rises with the run's own level is the genre's standard way of making later drafts feel better without a hard gate |
| D118 | Orchestrator decision (2026-09-23): lifetime achievements, tracked in the save profile and shown in the Hub, unlock new Draft cards (for example 300 lifetime kills unlocks Piercing Arrows) | Unlock cards through the Skill Tree only; or have everything available from the first run | The author's reference (Booster Pack Heroes) gates content behind quests; a small first-run pool keeps early drafts readable and gives long-term goals |
| D119 | Author decision (2026-09-23), from the author's own request ("when the player moves north over the tower it should go behind the tower ... create a depth effect"): the player, the Tower, and enemies move into one shared, Y-sorted play-layer z_index band (20) instead of the Tower's and player's own fixed bands (Tower 25, player 50); the freed z_index 50 becomes the always-on-top overhead health/shield/XP bar band | Keep the fixed Tower/player bands and special-case only the player-vs-Tower pair (e.g. a one-off Y comparison outside the draw-order system) | The author's request also has to hold for a goblin attacking the Tower's south face, not only the player; a special case for one pair does not generalize, and Godot 4's native nested Y-sort (the gameplay root and `Entities` both `y_sort_enabled`) gives the general behaviour for the cost of two z_index property edits |
| D120 | Orchestrator decision (2026-09-23), a direct consequence of D115: Mason's Kit (Tower branch, tier 2) is repointed from its original -15%/rank Tower Console repair price to +10%/rank Tower shield regeneration rate, since the Console it discounted no longer exists in a run | Retire the node and its Cores entirely (refund any rank already bought); or leave it as a documented no-op | A purchasable node that silently does nothing reads as a bug, not a placeholder (the same reasoning D114 gave for the Fortress capstone); shield regen rate is a Tower stat no other node yet touches, so it does not duplicate Shield Runes' own max-shield effect |
