...
A Godot 4.7.1 top-down 2D dual-entity survival roguelite ("Horde Control"): protect yourself and a central Tower at the same time. The game boots to `scenes/title.tscn` (title, controls, credits); Play loads `scenes/prototype.tscn` - arena, Tower, player with an auto-targeting weapon, three enemy intents, the HUD, the Wave Director's eight-wave sequence, pickups and the run economy, the Pressure Metric, upgrades, the Level-Up Draft, the Tower Console and run flow. Every Minimum Playable Prototype system is built; no review gate has run for Phases 02-05. Art is Pixel Frog's Tiny Swords (CC0 edition, decision D105) with frame-animated units, a grassland-island biome and music; the UI pass is merged (D106). Provenance for every third-party file is in `assets/third_party/*/PROVENANCE.md`.
