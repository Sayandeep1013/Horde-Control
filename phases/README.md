...
| 01 | Contracts, Core Documents, Harness | Typed data-contract schemas, documents 00-02 stable, headless test harness | P0.4, P0.6, P0.7 | 00 | Schema check (P0.6); Harness check (P0.7); doc lint clean on 00-02; the master's "Phase 0 accepted" row proposed to the author | In progress since 2026-09-18. Built, and review-gate iteration 1 run with four reviewers: P0.4 7/10, P0.6 8/10, P0.7 6/10, phase execution 8/10, eight Major findings. Three review-gate iterations run, the third against a tagged, frozen tree. Scores by iteration - P0.4 7/7/8, P0.6 8/9/9, P0.7 6/7/8, phase execution 8/7/8. Iteration 2's verdict was that sound artifacts were held down by the conduct of the review itself; iteration 3's reviewers recorded that as no longer true. Whether the phase meets its bar is not asserted here; the four reviewers' reports and the ledger are the record, and closure is the author's under the Gate Approval rule |
| 02 | Technical Foundations | The engine-level spine, proven under load | P1.1, P1.2, P1.3, P1.4, P1.5, P1.6, P1.7 | 01 | Keyed RNG unit check; Pause clock unit check; Registry query check; Cap unit check; Pool unit check; Recorder schema check; Ghost hit test; Audio priority check; Swarm performance test at a recorded Performance Fallback Ladder step; the master's "Phase 1 accepted" row proposed to the author | In progress since 2026-09-18. **Six of seven tasks built, independently verified and committed** - P1.1 through P1.6. **P1.7, the swarm stress test, is deferred by decision D97** so it measures real Phase 03 entities rather than placeholder capsules. **No review gate convened**; D98 defers it to a single gate on the finished prototype. Two open Majors carried: F02-09, the clustered query bound, and F02-16, the Ghost hit test being unable to catch its own defect class |
| 03 | Core Entities & Feel Check | Player, arena, weapon, Tower, three enemy intents, HUD, and a recorded feel verdict | P2.1, P2.2, P2.3, P2.4, P2.5, P2.6, P2.7 | 02 | Player movement check; Player silhouette test; Camera bounds test; Weapon check; Tower weapon check; Same-frame death test; Health recovery check; Leash test; Opportunist test; Stuck exemption test; Tower cue audibility check; Feel check with a recorded go verdict | Built out of sequence since 2026-09-19 under decisions D97-D100. P2.1 through P2.6 built, spot-checked and committed; **P2.8 was brought forward from Phase 04** when the author's first feel-check verdict named the missing wave pacing (F03-34). **P2.7 has been run twice by the author, and only the author can run it.** First verdict, 2026-09-19: adjust - no waves, entity art not good enough, empty arena. Second verdict, 2026-09-20, on the build carrying all three fixes: movement and shooting good, art still short, and the art pass deferred to its own session by D102. No review gate has been convened for this phase; D98 defers it to a single gate on the finished prototype |
| 04 | Wave Director & Pacing | Four encounter types from data, both spawn rings, the Pressure Metric, pickups | P2.8, P2.9, P2.10 | 03 | Spawn ring test; Split Assault test; Hunt test; Encounter recovery test; Wave runtime test; Overtime test; Pressure test; Pickup physics test | In progress since 2026-09-20. **P2.8 was built early, inside Phase 03** (F03-34); the part of its own Wave Runtime Model it deferred - the stall check, Overtime with finishers, encounter priority and deferred recovery gaps, the encounter alive cap, and the computed Siege volume formula - was built here as P2.8b, along with the Split Assault and Hunt suites P2.8 named but never wrote. P2.9 (Pressure Metric) and P2.10 (pickups, Drop Table, cap, run economy) built and falsified. Twenty findings recorded (F04-01 to F04-20), of which two are the author's: **F04-16, a numeric inconsistency inside the master itself** (the Siege volume formula yields six accompanying Hunters where the same Register row states seven), and F04-20, off-screen spawn markers left unbuilt because docs/11 never states what a marker renders. No review gate convened |
| 05 | Upgrades, Interfaces, Run Flow | Both upgrade channels and a run that can be played end to end | P2.11, P2.12, P2.13, P2.14 | 04 | Upgrade effect check; Draft queue test; Draft input lockout test; Guaranteed first draft test; Encounter deferral test; Determinism test; XP cap check; Console non-pause test; Console rules test; UI scaling test; Interaction window test; Focus loss test; Movement-only test; Teaching Siege tuning check; Scrap loss test; HUD layout check; Pause authority test; Run flow check | In progress since 2026-09-20, deliberately overlapping Phase 04 at the author's direction to build the rest of the game's functionality. **All four tasks built and falsified**: P2.11 (six upgrades and both fallback cards, ranks shared across both channels), P2.12 (Level-Up Draft), P2.13 (Tower Console, which never pauses), P2.14 (run flow, pause and settings menus, run-end screens, focus-loss pause). An integration pass then wired the assembled scene and **found a prototype-breaking defect by measurement**: a 14 px band between an attack's logical reach and its physical hitbox in which melee enemies attacked on schedule and their collision shapes never touched, so the Tower could not be damaged at all (F05-30). That pass was terminated early by an API session limit (F05-33); its work was salvaged, and three items it did not reach are carried - the assembled-scene assertion suite, the T4 re-measurement, and the headless run. Thirty-three findings recorded (F05-01 to F05-33). Decision D104 (the paused-menu timing carve-out) came out of this phase. **T4 remains untuned**: the Register row still carries the starting estimate, because every measurement taken before F05-30 was measuring a broken damage path. No review gate convened |
...
Two pieces of work do not sit where the phase table would put them. Both are recorded here so the table is not read as the whole truth.

| What | Where it belongs | Where it actually happened, or will | Why |
| --- | --- | --- | --- |
| **P2.8, the Wave Director** | Phase 04 | Built inside Phase 03 on 2026-09-19; its deliverables, its six named tests and its findings are in Phase 03's record (F03-34) | The author's first P2.7 feel-check verdict named the missing wave pacing as one of three adjustments. A feel check cannot be repeated against a build that does not have the thing the verdict asked for |
| **The art and asset session** | Phase 11 (P3.16, production art at slice density) | Its own session, after Phase 05's functionality lands and before the P2.16 external testers, per decision D102 | The author has now played the build twice and named the art both times. Prototype art is not P3.16's slice-density production art, but it has to carry itself in front of five people who have never seen the game, and spreading it across functional tasks produced three separate asset findings in Phase 03 (F03-21, F03-31, F03-32) |

Phases 04 and 05 are also deliberately **overlapping** rather than sequential: P2.11 depends on P0.6, P2.3 and P2.4 only, so it runs alongside P2.9 and P2.10 instead of waiting for them. P2.12, P2.13 and P2.14 do depend on that wave and follow it.
