...
- Settings check (P0.2), as amended by decision D82: the scripted audit `tests/settings_check.gd` runs against the project and asserts, by effective value, the 16 collision layer names in order, the full input map including number keys 1-7 and the debug toggles, every pinned project setting, the boot-check autoload and the export templates; and the exported Windows release build is launched to confirm the boot check fires inside it. A release-template build cannot host `--script`, which is why the audit itself does not run inside the build.
